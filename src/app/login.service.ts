...
const httpResponseOrErrorPartition = partition(
    (httpResponse) => httpResponse instanceof HttpErrorResponse === false
);
const shareWithError = <T>(src: Observable<T>) =>
    src.pipe(
        share(),
        catchError((err) => of(err))
    );
const postWithErrors = (http: HttpClient, url, body: any, options: any = {}) =>
    http
...
// .pipe((observable) => shareWithError(observable));
const getWithErrors = (http: HttpClient, url, options: any = {}) =>
    http.get(url, options).pipe((observable) => shareWithError(observable));
...
export class LoginService {
    ...
    isAuthorized$ = this.authorizedSubject
        .asObservable()
    ...
    login(credentials: RegisterCredentials | UserProfile) {
        const safeLogin$ = postWithErrors(
            this.http,
        ...
        success$
            ...
            .subscribe();
        error$.pipe(tap(() => this.authorizedSubject.next(false))).subscribe();
    ...
    check() {
        const [success$, error$] = httpResponseOrErrorPartition(
            getWithErrors(this.http, "http://localhost:5000/users/authorized")
        );
        success$
            .pipe(
                pluck("authorized"),
                tap((authStatus: boolean) =>
                    this.authorizedSubject.next(authStatus)
                )
            )
            .subscribe();
        error$
    ...
    info() {
        const [success$, error$] = httpResponseOrErrorPartition(
            this.http.get("http://localhost:5000/users/info").pipe(share())
        );
        success$
            .pipe(
