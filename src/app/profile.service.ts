...
export class ProfileService {
    public profileSubject = new BehaviorSubject(null) as BehaviorSubject<
        UserProfile
    >;
    ...
    updateProfile = (profile: any) => {
        ...
        const updateProfile$ = this.http
            .put(`http://localhost:5000/profiles/${_id}`, profile)
            .pipe(share());
        updateProfile$
            .pipe(
    ...
    requestProfile(id: any) {
        const objectObservable = this.http
            .get(`http://localhost:5000/profiles/${id}`)
            .pipe(share());
        objectObservable
            .pipe(
                tap((profile: UserProfile) => this.profileSubject.next(profile))
            )
            .subscribe();
    ...
    searchProfiles(params: { [key: string]: any }) {
        return this.http
            .get("http://localhost:5000/profiles", { params })
            .pipe(share());
    }

    queryProfiles(query: { [key: string]: any }) {
        return this.http
            .post("http://localhost:5000/profiles/q", query)
            .pipe(share());
    }
